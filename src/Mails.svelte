<script>
    ...
    const sortBy = (Array, sortOn, sortDir) => {
        const sortAttr1 = sortDir == 'asc' ? 1 : -1
        const sortAttr2 = sortDir == 'asc' ? -1 : 1

        switch(sortOn) {
            case 'date':
                return Array.sort((a, b) => (a.date > b.date) ? sortAttr1 : sortAttr2)
                break;
            case 'from':
                return Array.sort((a, b) => (a.from.toLowerCase() > b.from.toLowerCase()) ? sortAttr1 : sortAttr2)
                break;
            case 'to':
                return Array.sort((a, b) => (a.to[0].toLowerCase() > b.to[0].toLowerCase()) ? sortAttr1 : sortAttr2)
                break;
            case 'subject':
                return Array.sort((a, b) => (a.subject.toLowerCase() > b.subject.toLowerCase()) ? sortAttr1 : sortAttr2)
                break;
        }
    }
    
    let displayedMails = sortBy(mails, sortingProperty, sortingDirection)

    const handleSorting = (sort) => {
        sortingProperty = sort.detail.sortOn
        sortingDirection = sort.detail.sortDir
        displayedMails = sortBy(displayedMails, sortingProperty, sortingDirection)
    }

    const handleFilter = (filter) => {
        if(filter.detail.length > 0) {
            displayedMails = sortBy(filter.detail, sortingProperty, sortingDirection)
        } else {
            displayedMails = []
        }
    }
